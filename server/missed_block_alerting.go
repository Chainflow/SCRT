...
//SendSingleMissedBlockAlert to send alert when the validator miss the block
func SendSingleMissedBlockAlert(cfg *config.Config) error {
	...
	if !addrExists {
		_ = SendTelegramAlert(fmt.Sprintf("SCRT validator %s missed a block at block height %s", cfg.ValidatorName, cbh), cfg)
